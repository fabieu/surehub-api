...
async def get_households() -> List[official.Household]:
    return households.get_households()


@router.get("/{household_id}",
            response_model_exclude_none=True)
async def get_household_by_id(household_id: int) -> official.Household:
    return households.get_household_by_id(household_id)


@router.get("/{household_id}/users",
            response_model_exclude_none=True)
async def get_users_of_household(household_id: int) -> List[official.HouseholdUser]:
    return households.get_users_of_household(household_id)


@router.get("/{household_id}/users/{user_id}",
            response_model_exclude_none=True)
async def get_user(household_id: int, user_id: int) -> official.HouseholdUser:
    return households.get_user_of_household(household_id, user_id)


@router.get("/{household_id}/pets",
            response_model_exclude_none=True)
async def get_pets_of_household(household_id: int) -> List[official.Pet]:
...
@router.get("/{household_id}/devices",
            response_model_exclude_none=True)
async def get_devices_of_household(household_id: int) -> List[official.Device]:
