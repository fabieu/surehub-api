...
def get_households() -> List[official.Household]:
    ...
    return http_utils.extract_response_data(response, model=List[official.Household])
...
def get_household_by_id(household_id: int) -> official.Household:
    ...
    response = requests.get(uri, headers=auth.auth_headers())
    return http_utils.extract_response_data(response, model=official.Household)
...
def get_users_of_household(household_id: int) -> List[official.HouseholdUser]:
    ...
    response = requests.get(uri, headers=auth.auth_headers())
    return http_utils.extract_response_data(response, model=List[official.HouseholdUser])
...
def get_user_of_household(household_id: int, user_id: int) -> official.HouseholdUser:
    ...
    response = requests.get(uri, headers=auth.auth_headers())
    return http_utils.extract_response_data(response, model=official.HouseholdUser)
